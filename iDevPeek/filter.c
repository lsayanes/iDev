...
// Global variables
//
NDIS_HANDLE         FilterDriverHandle; // manejador del driver de filtro en NDIS.
NDIS_HANDLE         FilterDriverObject; //referencia al objeto driver en el sistema.
NDIS_HANDLE         NdisFilterDeviceHandle = NULL; //manejador del dispositivo NDIS para comunicación con user-mode.
PDEVICE_OBJECT      NdisDeviceObject = NULL; //referencia al objeto de dispositivo creado en el kernel.

/*

PDEVICE_OBJECT: Es una referencia directa a una estructura de datos de Windows que representa un dispositivo en el kernel. 
Se usa para interactuar con el sistema de archivos o con otros controladores.
...
--*/
{
    ...
    do
    {
        ...
        FChars.SendNetBufferListsHandler = NULL; //FilterSendNetBufferLists;
        FChars.ReturnNetBufferListsHandler = NULL; //FilterReturnNetBufferLists;
        FChars.SendNetBufferListsCompleteHandler = NULL; //FilterSendNetBufferListsComplete;
...
/*
        ...
        1. Define las funciones que manejarán los paquetes de red.
        2. Inicializa estructuras y nombres del driver.
        3. Registra el driver en NDIS.
        4. Crea un dispositivo para comunicación con user-mode.
...
--*/
{
    ...
    DEBUGP(DL_TRACE, "===>iDevPeek FilterPause: FilterInstance %p\n", FilterModuleContext);
...
--*/
{
    ...
    //Usar realmente la configuración obtenida con NdisOpenConfigurationEx o eliminar esa parte si no es necesaria.
...
--*/
{
    ...
    //
    // El filtro debe estar en estado FilterPaused antes de eliminarse.
    //
    FILTER_ASSERT(pFilter->State == FilterPaused);
    ...
    if (NULL != pFilter)
    {
        if (NULL != pFilter->FilterName.Buffer)
            FILTER_FREE_MEM(pFilter->FilterName.Buffer);


        //Eliminar la instancia del filtro de la lista global
        FILTER_ACQUIRE_LOCK(&FilterListLock, bFalse);
        ...
        DEBUGP(DL_TRACE, "<===FilterDetach pFilter == NULL\n");
...
--*/
{
    ...
    //
    iDevPeekDeregisterDevice();  //Desregistra el dispositivo si fue creado
    NdisFDeregisterFilterDriver(FilterDriverHandle); //Elimina el driver del filtro de NDIS

#if DBG

    /*
        Se asegura que la lista de módulos de filtro está vacía, lo que significa que todos los filtros fueron eliminados correctamente.    
    */
    ...
    FILTER_FREE_LOCK(&FilterListLock); //Elimina el FilterListLock, ya que el driver ya no lo necesita.
...
* 
* 
    Esta seccion OID FilterOidRequest()

    Esta función intercepta solicitudes de configuración al adaptador de red, como:
        * Cambiar la dirección MAC
        * Activar/desactivar VLANs
        * Modificar el tamaño del buffer (LookaheadSize)
        * Configurar el adaptador en modo promiscuo
        * Activar o desactivar características como checksum offload

    Por ahora el objetivo es capturar tráfico y enviarlo a la app, 
    entonces NO necesito modificar OIDs, porque no estoy cambiando la configuración del adaptador.
    Dejando Windows se encargará de pasar los OID automáticamente.


    Cuando sera necesaria?
    Si quiero que el filtro:

        * Habilite el modo promiscuo para capturar paquetes que no van dirigidos a la PC
        * Modifique el tamaño del buffer de captura (LookaheadSize)
        * Cambie algún parámetro de la red antes de reenviar la solicitud
*
* 
* 
* 
